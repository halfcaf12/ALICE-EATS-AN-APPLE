...
const char* SAVE_PATH = "/home/davidgn/gnn/alice_testing";
...
void storeCSV(const char* tree_name, TDirectoryFile* f) {
   const char* fname = f->GetName();
   int len = 50+strlen(fname)+strlen(tree_name)+5;
   char newfname[len];
   strcpy(newfname, SAVE_PATH);
   strcat(newfname, fname);
   strcat(newfname, tree_name);
...
void treeToCSV() {
   ...
   char pathname[60];
   const char* OPEN_DIR = "/eos/opendata/alice/masterclass/";
